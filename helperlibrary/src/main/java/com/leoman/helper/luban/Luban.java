...
public class Luban implements Handler.Callback {
    ...
    private void launch(final Context context) {
        if (mPaths == null || mPaths.size() == 0 && mCompressListener != null) {
            mCompressListener.onCompressError(new NullPointerException("image file cannot be null"), tag);
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (Checker.isImage(path)) {
                ...
                mCompressListener.onCompressError(new IllegalArgumentException("can not read the path : " + path), tag);
            }
            iterator.remove();
        }
    }
    ...
    public boolean handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_COMPRESS_START:
                mCompressListener.onCompressStart(tag);
                break;
            case MSG_COMPRESS_SUCCESS:
                mCompressListener.onCompressSuccess((File) msg.obj, tag);
                break;
            case MSG_COMPRESS_ERROR:
                mCompressListener.onCompressError((Throwable) msg.obj, tag);
                break;
        }
        return false;
    }
